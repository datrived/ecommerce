...
export const getUserDetail = (screenId) => async (dispatch, getState) => {
    ...
    try {
        ...
        const { data } = await axios.get(`/api/users/${screenId}/`,
                        config)
        ...
        console.log(error.response)
        dispatch({type: USER_DETAIL_FAIL, payload: error.response && error.response.data.detail ? 
            error.response.data.detail: error.message})
    }
}
...
export const updateUserProfile = (id, name, email, password) => async (dispatch, getState) => {
    ...
    try {
        ...
        const { data } = await axios.put(`/api/users/profile/update/`,
                        {'id': id, 'name': name, 'username': email, 'email': email, 'password': password},
                        config)
